...
public interface ExpenseSplitRepository extends JpaRepository<ExpenseSplit, UUID> {
    ...
    @Query(
            "SELECT tm.userId, COALESCE((SELECT SUM(e.amount) FROM Expense e WHERE e.tripId ="
                + " :tripId AND e.paidByUserId = tm.userId), 0) as totalPaid, COALESCE((SELECT"
                + " SUM(es.amount) FROM ExpenseSplit es JOIN Expense exp ON es.expenseId = exp.id"
                + " WHERE exp.tripId = :tripId AND es.userId = tm.userId AND es.isSettled = false),"
                + " 0) as totalOwed FROM TripMember tm WHERE tm.tripId = :tripId")
