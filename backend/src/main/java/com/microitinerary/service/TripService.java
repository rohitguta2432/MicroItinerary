...
import com.microitinerary.domain.GroupType;
...
import com.microitinerary.repository.InvitationRepository;
...
public class TripService {
    ...
    /** Update a trip */
    @Transactional
    public Optional<TripDetailResponse> updateTrip(
            UUID tripId, UUID userId, UpdateTripRequest request) {
        return tripRepository
                .findById(tripId)
                ...
                .map(
                        trip -> {
                            ...
                            if (existingUser.isPresent()
                                    && isMember(tripId, existingUser.get().getId())) {
                                throw new RuntimeException("User is already a member of this trip");
                            }

                            // Check if invitation already exists
                            Optional<Invitation> existingInvite =
                                    invitationRepository.findByTripIdAndEmail(tripId, email);
                            if (existingInvite.isPresent()
                                    && existingInvite.get().getStatus()
        ...
        return invitationRepository
                ...
                .map(
                        invitation -> {
                            ...
                            TripMember member =
                                    new TripMember(
                                            invitation.getTripId(), userId, MemberRole.MEMBER);
                            ...
                            return toDetailResponse(
                                    tripRepository.findById(invitation.getTripId()).orElseThrow());
                        });
    }
    ...
    private TripDetailResponse toDetailResponse(Trip trip) {
        ...
        List<TripMemberResponse> members =
                tripMemberRepository.findByTripId(trip.getId()).stream()
                        .map(
                                member -> {
                                    User user =
                                            userRepository
        ...
        return new TripDetailResponse(
                ...
                trip.getTravelType() != null ? trip.getTravelType().name() : null,
                trip.getGroupType() != null ? trip.getGroupType().name() : null,
                trip.getAmenities() != null
                        ? Arrays.asList(trip.getAmenities())
