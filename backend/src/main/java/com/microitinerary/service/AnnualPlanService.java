...
import com.microitinerary.dto.TripDtos.MonthData;
import com.microitinerary.dto.TripDtos.TripSummaryResponse;
import com.microitinerary.repository.AnnualPlanRepository;
...
public class AnnualPlanService {
    ...
    public AnnualPlanSummaryResponse createPlan(UUID userId, CreateAnnualPlanRequest request) {
        ...
        if (annualPlanRepository.existsByUserIdAndYear(userId, request.year())) {
            throw new RuntimeException(
                    "Annual plan for year " + request.year() + " already exists");
        }
    ...
    @Transactional
    public Optional<AnnualPlanSummaryResponse> updatePlan(
            UUID planId, UUID userId, UpdateAnnualPlanRequest request) {
        return annualPlanRepository
                .findById(planId)
                .filter(plan -> plan.getUserId().equals(userId))
                .map(
    ...
    public Optional<CalendarViewResponse> getCalendarView(UUID planId, UUID userId) {
        return annualPlanRepository
                ...
                .map(
                        plan -> {
                            List<Trip> trips =
                                    tripRepository.findByAnnualPlanIdOrderByStartDate(planId);

                            // Group trips by month
                            Map<Integer, List<Trip>> tripsByMonth =
                                    trips.stream()
                                            .collect(
                            ...
                            List<MonthData> months =
                                    IntStream.rangeClosed(1, 12)
                                            .mapToObj(
                                                    month -> {
                                                        List<TripSummaryResponse> monthTrips =
                                                                tripsByMonth
                                                                        .getOrDefault(
                                                                                month,
                                                                                Collections
                            ...
                            return new CalendarViewResponse(
                                    plan.getYear(), plan.getTotalBudget(), plannedCost, months);
                        });
    }
    ...
    private AnnualPlanDetailResponse toDetailResponse(AnnualPlan plan) {
        ...
        List<TripSummaryResponse> tripResponses =
                trips.stream().map(this::toTripSummary).collect(Collectors.toList());
