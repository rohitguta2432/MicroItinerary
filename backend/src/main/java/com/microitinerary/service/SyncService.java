...
public class SyncService {
    ...
    public void processPush(SyncPushRequest request) {
        for (SyncOp op : request.operations()) {
            switch (op) {
                case TripSyncOp tripOp -> handleTripOp(tripOp);
                case ActivitySyncOp activityOp -> handleActivityOp(activityOp);
                case PackingItemSyncOp packingOp -> handlePackingItemOp(packingOp);
            }
        }
    }
