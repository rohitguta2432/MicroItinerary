...
import com.microitinerary.dto.ExpenseDtos.BatchSettleRequest;
...
/** Expense Controller Endpoints for expense tracking and Splitwise-style splitting */
@RestController
@RequestMapping("/api")
public class ExpenseController {
    ...
    public ResponseEntity<TripExpenseSummaryResponse> getExpenseSummary(
        ...
        TripExpenseSummaryResponse summary =
                expenseService.getTripExpenseSummary(tripId, user.getId());
        if (summary == null) {
    ...
    public ResponseEntity<ApiResponse<Void>> settleSplit(
            @PathVariable UUID splitId, @AuthenticationPrincipal User user) {
    ...
    public ResponseEntity<ApiResponse<Integer>> batchSettle(
            @RequestBody BatchSettleRequest request, @AuthenticationPrincipal User user) {
        int settledCount = expenseService.batchSettle(request.splitIds(), user.getId());
        return ResponseEntity.ok(
                ApiResponse.success(settledCount + " splits settled", settledCount));
    }
}
