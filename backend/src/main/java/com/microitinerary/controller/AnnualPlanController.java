 ...
 * Endpoints for managing yearly travel plans
 */
@RestController
...
public class AnnualPlanController {
     ...
     */
    @GetMapping
    public ResponseEntity<List<AnnualPlanSummaryResponse>> getPlans(@AuthenticationPrincipal User user) {
     ...
     */
    @GetMapping("/current")
    public ResponseEntity<AnnualPlanSummaryResponse> getCurrentYearPlan(@AuthenticationPrincipal User user) {
    ...
    public ResponseEntity<AnnualPlanDetailResponse> getPlan(
        ...
        return annualPlanService.getPlanById(planId, user.getId())
    ...
    public ResponseEntity<CalendarViewResponse> getCalendarView(
            @PathVariable UUID planId,
            @AuthenticationPrincipal User user) {
        return annualPlanService.getCalendarView(planId, user.getId())
                .map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<AnnualPlanSummaryResponse> createPlan(
            @RequestBody CreateAnnualPlanRequest request,
            @AuthenticationPrincipal User user) {
        try {
    ...
    public ResponseEntity<AnnualPlanSummaryResponse> updatePlan(
            ...
            @AuthenticationPrincipal User user) {
        return annualPlanService.updatePlan(planId, user.getId(), request)
                .map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }
